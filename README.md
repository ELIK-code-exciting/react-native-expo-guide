...
- `<Text>Главная</Text>` - заголовок экрана (то что мы засунули в "коробку").
- `<StatusBar style = "auto"/>` - управляет цветом статус-бара.

### Теперь у нас есть тестовая страница, в будущем мы будем значительно её модернизировать.

**Вы также можете эксперементировать с текстом, например, добавив стиль к `<Text>` так, как это сделано с `<View>`.**

**В следующем файле я расскажу как создать навигационную панель** ===>>> [Как создать навигационную панель](TabNavigator.md).
